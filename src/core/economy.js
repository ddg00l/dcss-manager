...
export const ZUPGRADES = [
  { k: 'zatk',  n: 'Essence of Might',      d: '+20% damage for all heroes per lvl',   base: 2, g: 1.8, max: 20 },
  { k: 'zhp',   n: 'Essence of Fortitude', d: '+20% health for all heroes per lvl', base: 2, g: 1.8, max: 20 },
  { k: 'zloot', n: 'Essence of Greed',  d: '+20% gold per lvl',              base: 2, g: 1.8, max: 15 },
...
export const fameMul = s => 1 + s.fame.filter(f => f.won).length * 0.08;
...
export const ngMul = s => memHas(s, 'k_ngplus') ? 2.5 : 1;

export const gAtk  = s => (1 + memEff(s, 'atk')) * (1 + 0.2 * zupg(s, 'zatk')) * fameMul(s) * ghostMul(s) * runeAura(s);
export const gHp   = s => (1 + memEff(s, 'hp'))  * (1 + 0.2 * zupg(s, 'zhp'))  * fameMul(s);
export const gSpd  = s => 1 + memEff(s, 'spd');
export const gGold = s => (1 + memEff(s, 'gold')) * (1 + 0.2 * zupg(s, 'zloot')) * runeAura(s) * ngMul(s);
...
export const rollCost = s =>
  Math.floor(150 * Math.pow(1.18, s.rolls) * (1 - Math.min(0.5, memEff(s, 'gdisc'))));
