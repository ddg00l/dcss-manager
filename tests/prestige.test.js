...
describe('balance fixes from the 1000-session study', () => {
  it('each victory in the cycle hardens the dungeon', async () => {
    ...
    /* Isolate the in-cycle escalation: lifetime wins are held FIXED (they also
       feed fameMul, hence account power, hence the dungeon's power tracking) and
       only the cycle's own progress varies. */
    const LIFETIME = 40;
    const hpAt = (cycleWins) => {
      const s = makeState();
      s.stat.wins = LIFETIME;
      s.cycBase = { wins: LIFETIME - cycleWins, runes: 0, uniq: 0, mem: 0 };
    ...
    /* Hardening is geometric per Orb carried out this cycle, and it resets at
       ...
       DEMAND, which is why the prestige bar is capped (see PREST_CAP_ABS). */
